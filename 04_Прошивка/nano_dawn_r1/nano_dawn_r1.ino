/*
  ARDU DAWN R1 — DS3231 daily alarm + visible dawn on one WS2812B ring.
  ...
  R1 intentionally does NOT yet restore an in-progress dawn after Nano reset.
  That recovery behavior is a later FW-8 layer.
...
void startDawn(unsigned long durationMs, const __FlashStringHelper* source) {
...
void setup() {
  ...
  Serial.println(F("ARDU NANO DAWN R1 READY"));
  Serial.println(F("DAWN R1: MANUAL TEST + ALARM START; ACTIVE-DAWN RESET RECOVERY NOT YET ENABLED"));
  printStatus();
}
