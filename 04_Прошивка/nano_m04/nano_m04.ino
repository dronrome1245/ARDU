/*
  ARDU M04 — three-stripe frequency music mode for one 43-LED ring.

  Frequency grouping, noise gate, temporal smoothing and three-stripe layout
...
BandIndex bandForPixel(uint16_t index) {
  // Original ColorMusic 3-stripe layout:
  // first third HIGH, middle third MID, final third LOW.
  if (index < ArduConfig::LED_COUNT / 3) return BAND_HIGH;
  if (index < (ArduConfig::LED_COUNT * 2U) / 3U) return BAND_MID;
  return BAND_LOW;
}
...
void renderM04() {
  ...
  for (uint16_t i = 0; i < ArduConfig::LED_COUNT; ++i) {
    const BandIndex band = bandForPixel(i);
...
void printStatus() {
  Serial.print(F("STATUS FW=M04 MODE=M04 POWER="));
...
void setup() {
  ...
  Serial.println(F("ARDU NANO M04 READY"));
