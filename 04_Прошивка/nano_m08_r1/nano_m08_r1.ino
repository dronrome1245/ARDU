/*
  ARDU M08 R1 — running-frequency music mode with selectable band submodes.
...
constexpr uint8_t DEFAULT_RUNNING_SPEED_MS = 11;
...
struct ArduSettings {
  ...
  uint8_t runningSpeedMs = ArduConfig::DEFAULT_RUNNING_SPEED_MS;
...
bool runningFlag[BAND_COUNT] = {false, false, false};

unsigned long lastFrameMs = 0;
unsigned long lastRunningShiftMs = 0;
...
bool selectedRunningBand(BandIndex& band) {
  switch (settings.submode) {
    case Submode::THREE:
      // Original ColorMusic running-frequency priority: HIGH -> MID -> LOW.
      if (runningFlag[BAND_HIGH]) {
        band = BAND_HIGH;
        return true;
      }
      if (runningFlag[BAND_MID]) {
        band = BAND_MID;
        return true;
      }
      if (runningFlag[BAND_LOW]) {
        band = BAND_LOW;
        return true;
      }
      return false;
...
void shiftRunningTrailOutward() {
  constexpr uint16_t center = ArduConfig::LED_COUNT / 2;

  // Odd-count adaptation of the original linear-strip effect:
  // move the trail from the center toward both ends of the WS2812 chain.
  for (uint16_t i = 0; i < center; ++i) {
    leds[i] = leds[i + 1];
  }

  for (uint16_t i = ArduConfig::LED_COUNT - 1; i > center; --i) {
    leds[i] = leds[i - 1];
  }
}

void injectCenterPixel() {
  constexpr uint16_t center = ArduConfig::LED_COUNT / 2;
  BandIndex selected = BAND_LOW;

  if (selectedRunningBand(selected)) {
    leds[center] = CHSV(
        hueForBand(selected),
        255,
        bandBrightness[selected]
    );
  } else {
    leds[center] = CHSV(
        HUE_PURPLE,
...
void renderM08() {
  ...
  if (now - lastRunningShiftMs >= settings.runningSpeedMs) {
    lastRunningShiftMs = now;
    shiftRunningTrailOutward();
  }
...
void printStatus() {
  Serial.print(F("STATUS FW=M08 REV="));
  Serial.print(ArduConfig::FW_REV);
  Serial.print(F(" MODE=M08 POWER="));
  ...
  Serial.print(settings.runningSpeedMs);
...
void handleCommand(char* command) {
  ...
  if (strncmp(command, "SPEED ", 6) == 0) {
    ...
    settings.runningSpeedMs = value;
    Serial.print(F("OK SPEED_MS="));
    Serial.println(settings.runningSpeedMs);
    return;
  }
  ...
  if (strcmp(command, "HELP") == 0) {
    Serial.println(F("CMDS PING STATUS ADC CALF BANDS ON OFF SUBMODE SPEED BRIGHT BACKGROUND HELP"));
    Serial.println(F("FORMAT SUBMODE <THREE|LOW|MID|HIGH> | SPEED <1..255> | BRIGHT <0..255> | BACKGROUND <0..255>"));
    return;
  }
...
void setup() {
  ...
  Serial.println(F("ARDU NANO M08 R1 READY"));
