/*
  ARDU M05 R4 — one-strip frequency music mode with selectable band submodes.
...
void renderM05() {
  ...
  // Avoid starting a new LED frame while a Serial command is arriving.
  // With FASTLED_ALLOW_INTERRUPTS=1 the RX ISR remains available even if
  // the first byte arrives during an already-running WS2812B frame.
...
void setup() {
  ...
  Serial.println(F("ARDU NANO M05 R4 READY"));
