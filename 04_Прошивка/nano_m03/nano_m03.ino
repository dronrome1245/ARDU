/*
  ARDU M03 — five-stripe frequency music mode for one 43-LED ring.

  Frequency grouping, noise gate, temporal smoothing and five-stripe layout
...
BandIndex bandForZone(uint8_t zone) {
  // Original ColorMusic M03 layout: HIGH -> MID -> LOW -> MID -> HIGH.
  if (zone == 0 || zone == 4) return BAND_HIGH;
  if (zone == 1 || zone == 3) return BAND_MID;
  return BAND_LOW;
}
...
void renderM03() {
  ...
  for (uint16_t i = 0; i < ArduConfig::LED_COUNT; ++i) {
    // Proportional five-zone split covers all 43 LEDs.
    const uint8_t zone =
        static_cast<uint8_t>((static_cast<uint32_t>(i) * 5UL) /
                             ArduConfig::LED_COUNT);
    const BandIndex band = bandForZone(zone);
...
void printStatus() {
  Serial.print(F("STATUS FW=M03 MODE=M03 POWER="));
...
void handleCommand(char* command) {
  ...
  if (strcmp(command, "HELP") == 0) {
    Serial.println(F("CMDS PING STATUS ADC CALF BANDS ON OFF BRIGHT BACKGROUND HELP"));
...
void setup() {
  ...
  Serial.println(F("ARDU NANO M03 READY"));
